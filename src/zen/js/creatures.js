...
function updateNormalCreature(creature, neighbors, attraction, wanderAngle) {
    ...
    neighbors.forEach(other => {
        ...
        if (distance < 100) {
            ...
            if (distance < 25) {
                ...
                creature.targetAngle = repelAngle;
                creature.angle = creature.angle * 0.9 + creature.targetAngle * 0.1;
            }
        }
    });

    if (neighborCount > 0) {
        updateGroupBehavior(creature, centerX / neighborCount, centerY / neighborCount, 
    ...
    creature.targetAngle += wanderAngle * (creature.personality * 0.3);
}
...
function updatePosition(creature, deltaTime) {
    const maxAcceleration = creature.type === 'koi' ? 0.05 : 0.1;
...
function updateGroupBehavior(creature, centerX, centerY, avgDirection) {
